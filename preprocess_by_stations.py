# PURPOSE: mask, druncate, compare bold activity - REALTIME --> preprocess like every TR is new (up to that station)

# to do --figure out how to do best zscoring--for some reason even with good mask it doesn't perform good classification at the end but it did in the original way
# figure out why **
import glob
...
use_spline = 0 # whether or not to use spline at the end moco version
...
for s in np.arange(nSub):
  ...
  # FIRST -- calculate baseline zscored runs either R (1) to use on 2, 3,4 or run (2) to use on run 1

  # for each run load data 1 TR at a time--preprocessnormally UNTIL that TR 
  for r in np.arange(nruns):
    ...
    for t in np.arange(nTRs):
      ...
      if use_spline:
        ...
        transferred_image = glob.glob(new_tr_dir + '/' + this_tr_base + '_space-MNI.nii.gz')[0] 
    ...
    epi_fmriprep_original = nib.load(this_fmriprep_file).get_fdata()
    fmriprep_data_trunc = new_img_like(this_fmriprep_file,epi_fmriprep_original[:,:,:,n_trunc:])
    realtime_data_trunc = new_img_like(transferred_image,complete_new_nifti_like_image[:,:,:,n_trunc:])
    ...
    # don't remove story TRs yet before masking
    fmriprep_masked_data = epi_masker.fit_transform(fmriprep_data_trunc)
    fmriprep_originalData_all[:,:,r,s] = fmriprep_masked_data[story_TR_1-2:story_TR_2-2,:].T
    ...
    realtime_story_masked_Data = epi_masker.fit_transform(realtime_data_trunc)
    realtime_run_mean[:,r,s] = np.mean(realtime_story_masked_Data[story_TR_1-2:story_TR_2-2,:],axis=0)
    realtime_run_std[:,r,s] = np.std(realtime_story_masked_Data[story_TR_1-2:story_TR_2-2,:],axis=0,ddof=1)
    # this is the part that you would repeat after each station
    for st in np.arange(nStations):
      ...
      # make sure zscoring steps are all the same
      # TRY ZSCORING OTHER RUNS TOO TO SEE IF THAT HELPS OR IF IT'S THE FILTERING!
      # steps:
      # truncate 12 TRs to end
      # fit transform all data --> filter and mask
      # extract ONLY story TR's
      # zscore story TR's
      # remove average signal
      # to extract a previous run's signal: need voxel x story TRs x nruns
      # get this stations masked/filtered/average removed timecourse
      t_end = last_tr_in_station[st].astype(int)
      # need to see how it aligns to story TRs too!!!
      # only difference with this and above: stopping truncated version
      INCLUDE_EXTRA_TR=1
      realtime_data_trunc = new_img_like(transferred_image,realtime_story_truncated[:,:,:,0:t_end+INCLUDE_EXTRA_TR+story_TR_1-2]) # removing 12 TRs in the beginning
      realtime_masked_data = epi_masker.fit_transform(realtime_data_trunc)
      # now we zscore
      realtime_originalData_all[:,0:t_end,r,s,st] = realtime_masked_data[story_TR_1-2:t_end+story_TR_1-2,:].T
      ...
      # new version -- try to zscore another way
      
      #mean_non_story_data = np.mean(realtime_masked_data[0:story_TR_1-2,:],axis=0)
      #std_non_story_data = np.std(realtime_masked_data[0:story_TR_1-2,:],axis=0,ddof=1)
      #if r > 0:
      #  realtime_originalData_all[:,0:t_end,r,s,st] = np.nan_to_num((realtime_originalData_all[:,0:t_end,r,s,st] - realtime_run_mean[r,s])/realtime_run_std[r,s]
