...
realtime_allData = np.zeros((nVoxels,run_TRs,nRuns,nSub))
realtime_originalData_all = np.zeros((nVoxels,run_TRs,nRuns,nSub))
realtime_zscoredData_all = np.zeros((nVoxels,run_TRs,nRuns,nSub))
realtime_removedAvgData_all = np.zeros((nVoxels,run_TRs,nRuns,nSub))
non_brain_vox = {}
for s in np.arange(nSub):
  ...
  for r in np.arange(nruns):
    ...
    realtime_originalData_all[:,:,r,s] = realtime_masked_data[story_TR_1-2:story_TR_2-2,:].T
    ...
    realtime_originalData_all[:,:,r,s] = stats.zscore(realtime_originalData_all[:,:,r,s],axis=1,ddof = 1)
    realtime_originalData_all[:,:,r,s] = np.nan_to_num(realtime_originalData_all[:,:,r,s])
    realtime_zscoredData_all[:,:,r,s] = realtime_originalData_all[:,:,r,s]

    fmriprep_removedAvgData_all[:,:,r,s] = fmriprep_originalData_all[:,:,r,s] - average_signal
    realtime_removedAvgData_all[:,:,r,s] = realtime_originalData_all[:,:,r,s] - average_signal

    # this is just to compare voxel activity
    # # now compare voxel x time matrices for each data processing
    # z = pearsons_mean_corr(fmriprep_data_masked,all_tr_data_realtime)
    # A = fmriprep_data_masked
    # B = all_tr_data_realtime
    # num_cols = A.shape[1]
    # pearsonsList = []
    # for col in range(num_cols):
    #     A_col = A[:, col]
    ...
    # pearsons_mean = np.nanmean(pearsons[:, 0])
    # print(pearsons_mean)
    # badVox = np.argwhere(np.isnan(pearsons[:,0]))
    # print(badVox)
...
for s in np.arange(nSub):
  if s == 0:
    ...
    nruns = 4
  for r in np.arange(nRuns):
    z = pearsons_mean_corr(fmriprep_removedAvgData_all[:,:,r,s],realtime_removedAvgData_all[:,:,r,s])
    print('subject %i, run %i' % (s,r))
    print('mean corr = %2.2f' % (z))
...
for s in np.arange(nSub):
    ...
    for r in np.arange(nRuns):
        for st in np.arange(nStations):
            ...
            realtime_testing_data = realtime_removedAvgData_all[:,this_station_TRs,r,s]
